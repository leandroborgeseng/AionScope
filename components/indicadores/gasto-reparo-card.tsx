...
import { GastoBarChart, type GastoChartClickMeta } from "@/components/charts/charts";
import {
  ChartCard,
...
type CategoriaFiltro = GastoReparoCategoria | "todas";
...
const FICHA = FICHAS["gasto-reparo-medicos"];

function tituloListaGasto(
  drill: Drill,
  categoria: CategoriaFiltro,
  soComCusto: boolean,
) {
  if (!drill) return undefined;
  const base = drill.mesLabel
    ? categoria === "corretiva"
      ? `Reparos corretivos fechados em ${drill.mesLabel}`
      : categoria === "outro"
        ? `Man. externa / instrumental em ${drill.mesLabel}`
        : `Reparos fechados em ${drill.mesLabel}`
    : drill.title;
  return soComCusto ? `${base} — custo > 0` : base;
}
...
export function GastoReparoCard({
  ...
  const openMesLista = useCallback(
    (row: { name: string; year?: string | number; month?: string | number }, meta?: GastoChartClickMeta) => {
      const year = Number(row.year);
      const month = Number(row.month);
      const slot = gasto.months.find((m) => m.year === year && m.month === month);
      const cat: CategoriaFiltro =
        meta?.dataKey === "gastoCorretiva" ? "corretiva" : meta?.dataKey === "gastoOutro" ? "outro" : "todas";
      setSoComCusto(false);
      setCategoria(cat);
      setDrill({
  ...
  const openPeriodo = useCallback(
    (rows: GastoReparoRow[], title: string, subtitle: string, opts?: { soComCusto?: boolean; categoria?: CategoriaFiltro }) => {
      setSoComCusto(opts?.soComCusto ?? false);
      setCategoria(opts?.categoria ?? "todas");
  ...
  const drillVisible = useMemo(() => {
    if (!drill) return [];
    let rows = drill.rows;
    if (categoria === "corretiva") rows = rows.filter((item) => item.categoria === "corretiva");
    if (categoria === "outro") rows = rows.filter((item) => item.categoria === "outro");
    return soComCusto ? gastoReparoComCusto(rows) : rows;
  }, [drill, categoria, soComCusto]);
  ...
  const listaTitle = tituloListaGasto(drill, categoria, soComCusto);

  const filters = drill ? (
    <>
      <FilterChip
        active={categoria === "todas" && !soComCusto}
        onClick={() => {
          setCategoria("todas");
          setSoComCusto(false);
        }}
  ...
  const detalhesItems: Array<{ id: string; title: string; children: ReactNode }> = [
    {
      id: "recorte",
      title: "Recorte: reparo de equipamentos médicos",
      children: (
        <div className="space-y-3 text-slate-700">
          ...
          <p className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-amber-900">
            Calibração, TSE e preventiva <strong>não são reparo</strong> — ficam de fora desta soma. OS sem Fechamento e
            sem DataDaSolucao também ficam de fora: o reparo ainda não foi concluído.
          </p>
        </div>
      ),
    },
        ...
        <div className="space-y-4 text-slate-700">
          <dl className="grid gap-3 sm:grid-cols-2">
            ...
            <OrigemCampo label="Filtro local">
              Tag no índice médico + tipo de reparo + Fechamento/DataDaSolucao entre {range.fromISO} e {range.toISO}. OS
              sem data de fechamento não entram no gasto do mês.
            </OrigemCampo>
            ...
            <OrigemCampo label="Após filtro" valueClassName="mt-1 text-sm font-semibold tabular-nums leading-relaxed">
              {gasto.aposMedico.length} com tag médica · {gasto.aposReparo.length} reparo · {gasto.noIntervalo.length}{" "}
              fechadas no intervalo
            </OrigemCampo>
          </dl>
          ...
          <div>
            ...
            <p>
              <strong>Gasto do mês</strong> = soma de <code className="rounded bg-slate-100 px-1">Custo</code> das OS que
              passam no recorte e cuja data de fechamento cai naquele mês.
            </p>
          </div>
          ...
          <div>
            ...
            <ul className="list-disc space-y-1 pl-5">
              ...
              <li>
                ...
                <strong className="tabular-nums">{gasto.semFechamento.length}</strong> — excluídas do gasto (ainda não
                concluídas).
              </li>
              <li>
            ...
            >
              ...
              {gasto.osCustoZero} vêm {gasto.osCustoZero === 1 ? "zerada" : "zeradas"}. O gráfico{" "}
              <strong>não esconde</strong> esses zeros — eles entram na soma como R$ 0.
            </p>
            {gasto.empilhar ? (
              <p className="mt-2 text-slate-600">
                Há duas categorias com gasto &gt; 0 no período (corretiva {formatBRL(gasto.gastoCorretiva)} · man. externa
                / instrumental {formatBRL(gasto.gastoOutro)}), então as barras empilham.
              </p>
            ) : (
              <p className="mt-2 text-slate-600">
          ...
          <div>
            ...
            <p>
              Busque pelo código da OS. Confira Tag (equipamento médico), Tipo de manutenção (corretiva / A -, sem ser
              preventiva, calibração ou TSE), data de fechamento e o campo Custo.
            </p>
          </div>
      ...
      <IndicadorPageLayout
        ...
        kpis={
          <>
            <KpiCard
              ...
              onClick={() =>
                openPeriodo(gasto.noIntervalo, `Gasto · ${range.label}`, "OS de reparo médico com fechamento no intervalo")
              }
            />
            <KpiCard
              label="Média mensal"
              ...
              onClick={() =>
                openPeriodo(
                  gasto.noIntervalo,
                  `Gasto · ${range.label}`,
                  "Mesma lista do gasto total — a média divide pelos meses do intervalo",
                )
              }
            />
            <KpiCard
              ...
              onClick={() =>
                openPeriodo(
                  gastoReparoComCusto(gasto.noIntervalo),
                  `OS com custo > 0 · ${range.label}`,
                  "Reparo médico fechado no intervalo e Custo parseado maior que zero",
                  { soComCusto: true },
                )
              }
            />
        ...
        chart={
          <ChartCard
            ...
            hint="Barra = soma do Custo no mês de fechamento. Clique no mês (ou no segmento, se empilhado) para listar as OS abaixo."
          >
            <GastoBarChart data={chartData} xKey="name" stacked={gasto.empilhar} onRowClick={openMesLista} />
          </ChartCard>
        }
        selectionList={
          <SelecaoOsLista
